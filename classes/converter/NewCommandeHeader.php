...
class NewCommandeHeader extends Converter
{
    ...
    public function converter_for_db()
    {
        ...
        $this->data_for_db["magasin_uid"] = \array_key_exists("stat", $input) ? \intval($input["stat"]) != "" ?  \intval($input["stat"]) : \null : \null;

        $this->data_for_db["state"] = \array_key_exists("state", $input) ? \intval($input["state"]) != "" ?  \intval($input["state"]) : 1 : 1;

        $this->data_for_db["taux_remise"] = \array_key_exists("remise-taux", $input) ? \floatval($input["remise-taux"]) != "" ?  \floatval($input["remise-taux"]) : 0 : 0;

        $this->data_for_db["montant_remise"] = \array_key_exists("remise-montant", $input) ? \floatval($input["remise-montant"]) != "" ?  \floatval($input["remise-montant"]) : 0 : 0;

        $this->data_for_db["total_ht"] = \array_key_exists("totalHT-avant-remise", $input) ? \floatval($input["totalHT-avant-remise"]) != "" ?  \floatvaltrim($input["totalHT-avant-remise"]) : 0 : 0;
        ...
        $this->data_for_db["total_ttc"] = \array_key_exists("totalTTC-apres-remise", $input) ? \floatval($input["totalTTC-apres-remise"]) != 0 ?  \floatval($input["totalTTC-apres-remise"]) : 0 : 0;
    }
}
