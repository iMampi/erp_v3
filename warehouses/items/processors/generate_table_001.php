...
if (can_visit($cycle_item)) {
    require_once $_SERVER["DOCUMENT_ROOT"] . "/database/select/all_items_name_limit.php";
    $count_is_bad = \true;
    try {
        $count_is_bad = !count($all_items_name_limit);
    } catch (\Throwable $th) {
    }
    if ($count_is_bad) {
        ...
        foreach ($all_items_name_limit as $item) {
            // echo "itesm";
            // var_dump($item);
